...
export function DemoView({ tableNumbers }: { tableNumbers: number[] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-10 px-6 py-10 lg:flex-row lg:items-start lg:justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <PhoneFrame key={phoneKey} src={`/order/${selectedTable}`} label="客のスマホ画面" />
      </div>

      <TabletFrame src="/staff/orders" label="店舗スタッフ用タブレット画面" />
    </div>
  );
}
